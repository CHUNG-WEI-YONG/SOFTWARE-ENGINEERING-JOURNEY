...
def understand_prompt_node(state:SearchState):
    user_message = ""
...
def search_node(state:SearchState):
    search_req=state["search_query"]
    try:
        print(f"🔍 正在搜索: {search_req}")
        response=tavily_client.search(query=search_req,search_depth="basic",include_answer=True,max_results=5)
        search_results = ""
...
def generate_answer_node(state:SearchState):
    if state["steps"]=="search_failed":
        fallback_prompt = f"搜索API暂时不可用，请基于您的知识回答用户的问题：\n用户问题：{state['user_query']}"
        rsp=llm.invoke([SystemMessage(content=fallback_prompt)])
    else:
        answer_prompt = f"""基于以下搜索结果为用户提供完整、准确的答案：
用户问题：{state['user_query']}
搜索结果：\n{state['search_results']}
请综合搜索结果，提供准确、有用的回答..."""
        rsp=llm.invoke([SystemMessage(content=answer_prompt)])
    return {
        "final_answer": rsp.content,
        "steps": "completed",
        "messages": [AIMessage(content=rsp.content)]
    }
...
def create_search_assistant():
    ...
    workflow.add_node("understand",understand_prompt_node)
