...
def get_name(character:str)->str:
    if character and character in CHINESE_NAMES:
        return character
    else:
        return random.choice(CHINESE_NAMES)

def show_player(players:list[str],show:bool=False):
    if not players:
        return "No players"
    if show:
...
def check_win(alive_player:List[AgentBase],role:Dict[str,str]):
    roles=[role.get(p.name,"村民")for p in alive_player]
    werewolf_count = roles.count("狼人")
    villager_count = len(roles) - werewolf_count
...
def analysis_speech(speech:str)->Dict[str,Any]:
    analysis = {
...
class GameModerator(AgentBase):
    ...
    async def announce(self,announce:str):
        message=Msg(
            name=self.name,
            role='system',
            content=message
    ...
    async def death_announcement(self,dead_players:List[str]):
        if not dead_players:
            content = "昨夜平安无事，无人死亡。"
        else:
...
def format_player_list_str(players:List[str]):
    if not players:
        return "无人"
    return "、".join(players)
