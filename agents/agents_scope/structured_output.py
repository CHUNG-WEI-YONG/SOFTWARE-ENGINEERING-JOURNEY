...
def get_vote_model_cn(agents: list[AgentBase]) -> type[BaseModel]:
    ...
    # 1. Extract agent names into a tuple
    agent_names = tuple(_.name for _ in agents)

    # 2. Dynamically construct the Literal type using tuple unpacking
    VoteTargetType = Literal[agent_names]  # Unpacks ('Alice', 'Bob', ...)

    # 3. Use create_model to dynamically build the Pydantic class
    return create_model(
        "VoteModelCN",
        vote=(
            VoteTargetType,
            Field(description="你要投票淘汰的玩家姓名"),
        ),
        reason=(
            str,
            Field(description="投票理由，简要说明为什么选择此人"),
        ),
        suspicion_level=(
            int,
            Field(
...
def get_seer_model_cn(agents: list[AgentBase]) -> type[BaseModel]:
    ...
    # 1. 提取当前可查验的玩家姓名元组
    agent_names = tuple(_.name for _ in agents)
    TargetType = Literal[agent_names]
    ...
    return create_model(
        ...
        target=(
            TargetType,
            Field(description="要查验的玩家姓名"),
        ),
        check_reason=(
            str,
            Field(description="查验此人的原因"),
        ),
        priority_level=(
            int,
            Field(description="查验优先级(1-10)", ge=1, le=10),
        ),
    )
...
def get_hunter_model_cn(agents: list[AgentBase]) -> type[BaseModel]:
    ...
    return create_model(
        ...
        shoot=(
            bool,
            Field(description="是否使用开枪技能"),
        ),
        target=(
            Optional[TargetType],
            Field(default=None, description="开枪目标玩家姓名"),
        ),
        shoot_reason=(
            Optional[str],
            Field(default=None, description="开枪理由"),
        ),
    )
